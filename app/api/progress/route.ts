 ...
 *
 * Auth: the client sends the passphrase in the x-gcptrainer-passphrase
 * header; it must match the GCPTRAINER_PASSPHRASE environment variable.
...
function isAuthorized(request: NextRequest): boolean {
  const expected = process.env.GCPTRAINER_PASSPHRASE;
  if (!expected) return false; // sync disabled until configured
  const given = request.headers.get("x-gcptrainer-passphrase") ?? "";
