...
export default function ReviewPage() {
  ...
  // Guarded, one-shot: computed on the render where `ready` first becomes
  // true, then left alone (see the comment on `queue` above). Calling
  // setState conditionally during render, rather than from an effect, is
  // the pattern React recommends for this "run once" reset:
  // https://react.dev/reference/react/useState#storing-information-from-previous-renders
  if (ready && queue === null) {
    setQueue(dueCardIds(progress, [...cardsById.keys()], toDayKey(new Date())));
  }
