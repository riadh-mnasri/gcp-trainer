...
Date: 2026-07-10
Status: accepted (inherited from the Kortex template on 2026-07-13)

## Context

GcpTrainer is built from the Kortex template, which was designed as a
reference for future learning apps (Rust, GCP, ...). If lessons or
GCP-specific logic leak into components, cloning the app for another
topic means untangling code.
...
- **Engine** (`lib/engine`, `components`, `app`): spaced repetition,
  ...
  defined in `lib/content/types.ts` and never references GCP.
...
- Content is data: it can be reviewed, diffed and generated (see the
  gcp-news skill) without touching the engine.
