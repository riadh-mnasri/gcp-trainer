...
const STORAGE_KEY = "gcptrainer.progress.v1";
const PASSPHRASE_KEY = "gcptrainer.passphrase";
...
export function ProgressProvider({ children }: { children: ReactNode }) {
  const [progress, setProgress] = useHydratedState(getPersistedProgress, EMPTY_PROGRESS);
  // False on the server and on the first client render, true right after
  // hydration (localStorage has then been read, see useHydratedState above).
  const ready = useSyncExternalStore(noopSubscribe, () => true, () => false);
  const [syncStatus, setSyncStatus] = useState<SyncStatus>("local");
  const pushTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Startup: reconcile the local copy with the server copy.
  useEffect(() => {
    const passphrase = window.localStorage.getItem(PASSPHRASE_KEY) ?? "";
    if (!passphrase) return;
    const local = getPersistedProgress();
...
/**
 * Cached wrapper around loadLocal(): useSyncExternalStore requires a
 * stable reference across calls when nothing changed, and this app is
 * the sole writer of gcptrainer.progress.v1, so reading once is enough.
 */
let cachedLocal: Progress | null = null;
function getPersistedProgress(): Progress {
  if (cachedLocal === null) cachedLocal = loadLocal();
  return cachedLocal;
}

function saveLocal(progress: Progress): void {
  window.localStorage.setItem(STORAGE_KEY, JSON.stringify(progress));
  cachedLocal = progress;
}
...
async function pullFromServer(passphrase: string): Promise<Progress | null> {
  try {
    const res = await fetch("/api/progress", {
      headers: { "x-gcptrainer-passphrase": passphrase },
    });
  ...
  try {
    const res = await fetch("/api/progress", {
      ...
      headers: {
        ...
        "x-gcptrainer-passphrase": passphrase,
      },
